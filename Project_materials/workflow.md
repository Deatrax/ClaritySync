This is a fantastic approach. Since this is an **RDBMS course project**, shifting logic from the application layer (Node.js) into the database layer (SQL Functions/Triggers) is exactly how you get an A+. It demonstrates that you understand the power of the database engine.

We will use **Supabase**, which is just PostgreSQL under the hood. You can run the entire script below in the Supabase "SQL Editor" to set up your entire backend logic in one go.

### 1. The Master SQL Script (Schema + Logic)

This script does everything: creates tables, defines the "Action Functions" (for Sales/Stock), and sets up "Automation Triggers" (for Balances).
...
### 2. Implementation & Task Assignment (Next.js + Node)

Since you are using Next.js (Frontend) and Node (Backend), your architecture is:
`Frontend Form (Next.js)` → `API Call (fetch)` → `Node Express Route` → `Supabase SQL Function`.

#### 📄 Page 1: The Dashboard (`/pages/index.tsx`)

**Owner:** Shared
**Purpose:** Quick navigation and "At a glance" stats.

* **UI Components:**
* **Hero Section:** 3 Large Cards ("Total Products", "Total Customers", "Cash in Hand").
* **Quick Actions:**
* Button: "New Sale" (Links to `/pos`).
* Button: "Add Purchase/Stock" (Links to `/inventory/add`).


* **Search Bar:** Large text input center screen. "Search Product or Customer...".


* **Backend Requirement:**
* `GET /api/dashboard/stats`: Returns count of products, contacts, and sum of banking balance.



#### 📄 Page 2: Inventory Manager (`/pages/inventory/index.tsx`)

**Owner:** Anjim (Inventory Logic)
**Purpose:** Define categories, products, and buy stock (Feature 1, 2, 3).

* **UI Components:**
* **Tabs:** [Product List] | [Add New Product] | [Add Stock]
* **Form (Add Product):**
* Dropdown: Category (Fetch from `category` table).
* Input: Product Name, Brand.
* Checkbox: "Has Serial Number?".


* **Form (Add Stock/Purchase):**
* Dropdown: Supplier (Fetch from `contacts` WHERE type='SUPPLIER').
* Dropdown: Product.
* Input: Serial Number (Show only if product is serialized).
* Input: Cost Price (Buying Price).
* Dropdown: Paid From Account (Fetch `banking_account` e.g., "Cash Till").




* **SQL Connection:**
* The "Add Stock" form **MUST** call the `sp_add_stock` stored procedure via the backend.



#### 📄 Page 3: Point of Sale (POS) (`/pages/sales/new.tsx`)

**Owner:** Didhiti (Sales Logic)
**Purpose:** The Cash Register (Feature 4, 5, 6).

* **UI Components:**
* **Layout:** Two columns. Left = Product Search/Grid. Right = Cart/Bill.
* **The Cart:**
...
* **Checkout Section (Crucial):**
* **Customer Toggle:** Radio Button [Walk-in] vs [Registered].
* **If Registered:** Search bar to find customer by Phone/Name.
* **Payment Method:** Radio Button [Cash] | [Bank] | [Due/Ledger].
* **Button:** "Complete Sale" -> On success, show "Print Receipt" button.




* **SQL Connection:**
* This page sends a JSON payload to `sp_process_sale`.



#### 📄 Page 4: Customer Directory (`/pages/contacts/index.tsx`)

**Owner:** Sadman/Anjim
**Purpose:** View Dues and Histories (Feature 8, 9, 10).

* **UI Components:**
* **Table:** Columns [Name, Phone, Type, **Current Due/Balance**].
* **Sorting:** Dropdown "Sort by: Highest Due First".
* **Action:** Clicking a row goes to `/pages/contacts/[id].tsx`.


* **Detail Page (`[id].tsx`):**
* Shows "Profile Card" (Name, Address).
* Shows "Transaction History" table (The UNION query provided in the previous response).



---

### 3. Immediate "Next Step" for Thursday Presentation

You have a lot of code here. To survive the presentation, **don't build the whole UI.** Build just these two forms:

1. **Inventory Form:** To add a "MacBook" and buy 5 units. (Proves `sp_add_stock` works).
2. **POS Form:** To select that MacBook and sell it to "Customer A" on "Due". (Proves `sp_process_sale` and Ledger logic works).

**Would you like me to write the Next.js (React) code for the "Add Stock" form component so Anjim has a starting point?**